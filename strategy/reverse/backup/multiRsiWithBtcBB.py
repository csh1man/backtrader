        ...
        try:
            ...
            for i in range(1, len(self.pairs)):
                ...
                if self.btc_close[0] > self.bb_top[0]:
                    for j in range(0, 5):
                        price = Decimal(self.pair_close[i][0]) * (Decimal('1') - self.p.bull_percents[j] / Decimal('100'))
                        price = int(price / self.p.tick_size[i]) * self.p.tick_size[i]
                        price = price.quantize(self.p.tick_size[i], rounding=ROUND_HALF_UP)
                        prices.append(float(price))
                elif self.bb_bot[0] < self.btc_close[0] < self.bb_top[0]:
                    for j in range(0, 5):
                        price = Decimal(self.pair_close[i][0]) * (
                                    Decimal('1') - self.p.default_percents[j] / Decimal('100'))
                        price = int(price / self.p.tick_size[i]) * self.p.tick_size[i]
                        price = price.quantize(self.p.tick_size[i], rounding=ROUND_HALF_UP)
                        prices.append(float(price))
                elif self.btc_close[0] < self.bb_bot[0]:
                    for j in range(0, 5):
                        price = Decimal(self.pair_close[i][0]) * (Decimal('1') - self.p.bear_percents[j] / Decimal('100'))
                        price = int(price / self.p.tick_size[i]) * self.p.tick_size[i]
                        price = price.quantize(self.p.tick_size[i], rounding=ROUND_HALF_UP)
                        prices.append(float(price))
                ...
                for j in range(0, 5):
                    ...
                    qty = int(qty / self.p.step_size[i]) * self.p.step_size[i]
                    if qty > 0:
...
if __name__ == '__main__':
    ...
    for pair, tick_kind in pairs.items():
        ...
        df = DataUtils.get_candle_data_in_scape(df, start_date, end_date)
    ...
    qs.reports.html(returns, output=f'xrpusdt and dogeusdt.html', title='result')