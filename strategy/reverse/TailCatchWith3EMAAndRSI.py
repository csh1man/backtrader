...
result_file_prefix = "TailCatchWith3EMAAndRSI"
...
class TailCatchWith3EMAAndRSI(bt.Strategy):
    ...
    def __init__(self):
        ...
        for i in range(0, len(self.pairs)):
            ema1 = bt.indicators.ExponentialMovingAverage(self.closes[i], period=self.p.ma_length[0])
            self.ema1.append(ema1)

            ema2 = bt.indicators.ExponentialMovingAverage(self.closes[i], period=self.p.ma_length[1])
            self.ema2.append(ema2)

            ema3 = bt.indicators.ExponentialMovingAverage(self.closes[i], period=self.p.ma_length[2])
