...
class TailCatchWithEMAV1(bt.Strategy):
    ...
    # def stop(self):
    #     sorted_top_percents = sorted(self.bull_percents, key=lambda x: x[1])
    #     sorted_mid_percents = sorted(self.def_percents, key=lambda x: x[1])
    #     sorted_bot_percents = sorted(self.bear_percents, key=lambda x: x[1])
    #     self.log(f"bull percents length : [{len(sorted_top_percents)}]")
    #     for key, value in sorted_top_percents[:51]:
    #         self.log(f'{key} -> {value}%')
    #     self.log("")
    #
    #     self.log(f"default percents length : [{len(sorted_mid_percents)}]")
    #     for key, value in sorted_mid_percents[:51]:
    #         self.log(f'{key} -> {value}%')
    #     self.log("")
    #
    #     self.log(f"bear percents length : [{len(sorted_bot_percents)}]")
        ...
        for i in range(0, len(self.pairs)):
            ...
            if current_position_size > 0:
                if self.rsi[i][0] >= self.p.rsi_limit[name]:
                    self.order = self.sell(exectype=bt.Order.Limit, data=self.pairs[i], price=self.closes[i][0], size=float(current_position_size))

            percents = self.p.percents[name]['def']
            if self.ema1[i][-1] >= self.ema2[i][-1] >= self.ema3[i][-1]:
                percents = self.p.percents[name]['bull']
            elif self.ema1[i][-1] < self.ema2[i][-1] < self.ema3[i][-1]:
            ...
            # if self.ema1[i][-1] >= self.ema2[i][-1] >= self.ema3[i][-1]:
            #     self.bull_percents.append((date, candle_percent))
            # elif self.ema1[i][-1] < self.ema2[i][-1] < self.ema3[i][-1]:
            #     self.bear_percents.append((date, candle_percent))
            # else:
            #     self.def_percents.append((date, candle_percent))
