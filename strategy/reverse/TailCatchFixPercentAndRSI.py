...
result_file_prefix = "TailCatchFixPercentAndRSI"
...
class TailCatchWith3EMAAndRSI(bt.Strategy):
    params=dict(
        log=True,
        risks=[Decimal('0.1'), Decimal('0.5'), Decimal('1'), Decimal('2'), Decimal('3'), Decimal('4'), Decimal('5'), Decimal('10'), Decimal('15'), Decimal('20')],
        percents={
            'bull': [Decimal('1'), Decimal('2'), Decimal('3'), Decimal('5'), Decimal('8'), Decimal('10'), Decimal('12'), Decimal('15'), Decimal('20'), Decimal('25')],
            'def': [Decimal('2'), Decimal('3'), Decimal('4'), Decimal('6'), Decimal('8'), Decimal('10'), Decimal('12'), Decimal('15'), Decimal('20'), Decimal('25')],
            'bear': [Decimal('2'), Decimal('3'), Decimal('5'), Decimal('7'), Decimal('10'), Decimal('12'), Decimal('15'), Decimal('18'), Decimal('20'), Decimal('25')]
        },
        rsi_length=3,
        rsi_limit=40,
        wpr_length=14,
        ...
        tick_size={
            ...
            'AAVEUSDT': common.fetch_tick_size(exchange, 'AAVEUSDT'),
        ...
        step_size={
            ...
            'AAVEUSDT': common.fetch_step_size(exchange, 'AAVEUSDT'),
    ...
    def __init__(self):
        ...
        for i in range(0, len(self.pairs)):
            ...
            wpr = bt.indicators.WilliamsR(self.pairs[i], period=self.p.wpr_length)
            self.wpr.append(wpr)

            ema1 = bt.indicators.ExponentialMovingAverage(self.pairs[i], period=self.p.ma_length[0])
            self.ema1.append(ema1)

            ema2 = bt.indicators.ExponentialMovingAverage(self.pairs[i], period=self.p.ma_length[1])
            self.ema2.append(ema2)

            ema3 = bt.indicators.ExponentialMovingAverage(self.pairs[i], period=self.p.ma_length[2])
    ...
    def next(self):
        ...
        for i in range(0, len(self.pairs)):
            ...
            if current_position_size > 0:
                ...
                # if self.wpr[i][0] < self.p.wpr_limit:
            ...
            percents = self.p.percents['def']
            if self.ema1[i][0] >= self.ema2[i][0] >= self.ema3[i][0]:
                percents = self.p.percents['bull']
            elif self.ema1[i][0] <= self.ema2[i][0] <= self.ema3[i][0]:
                percents = self.p.percents['bear']
...
if __name__ == '__main__':
    cerebro = bt.Cerebro()
    cerebro.addstrategy(TailCatchWith3EMAAndRSI)
