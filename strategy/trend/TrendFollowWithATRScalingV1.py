...
class TrendFollowWithATRScalingV1(bt.Strategy):
    ...
    def next(self):
        ...
        for i in range(0, len(self.pairs)):
            ...
            # self.log(f'{self.dates[i].datetime(0)} => long high band : [{long_high_band}], long low band : [{long_low_band}]')
            # self.log(f'{self.dates[i].datetime(0)} => adj long high band : {adj_long_high_band}, adj long low band : {adj_long_low_band}')
            '''
            숏 고가/저가 채널 생성
            '''
