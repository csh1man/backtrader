...
class UpbitBBWithTrendFollowV2(bt.Strategy):
    params=dict(
        log=True,
        risk={
            'KRW-ETH': Decimal('2.0'),
    ...
    def __init__(self):
        ...
        for i in range(0, len(self.pairs)):
            ...
            ma1 = bt.indicators.ExponentialMovingAverage(self.closes[i],period=self.p.ma_length[name][0])
            self.ma1.append(ma1)

            ma2 = bt.indicators.ExponentialMovingAverage(self.closes[i], period=self.p.ma_length[name][1])
            self.ma2.append(ma2)
    ...
    def next(self):
        ...
        for i in range(0, len(self.pairs)):
            ...
            if current_position_size == 0:
                if self.closes[i][-1] < self.bb_top[i][-1] and self.closes[i][0] >= self.bb_top[i][0] and self.ma1[i][0] >= self.ma2[i][0]:
                    stop_price = DataUtils.convert_to_decimal(self.adj_low_bands[i][0])
                    stop_price = int(stop_price / tick_size) * tick_size

                    qty = DataUtils.convert_to_decimal(self.broker.getcash()) * self.p.risk[name] / Decimal('100')
...
if __name__ == '__main__':
    ...
    cerebro.addstrategy(UpbitBBWithTrendFollowV2) # 전략 추가
    ...
    for pair, tick_kind in pairs.items():
        file_name += pair + "-"
    file_name += "UpbitBBWithTrendFollowV2"
