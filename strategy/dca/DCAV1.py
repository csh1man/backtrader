...
class TailCatchExitFastV2(bt.Strategy):
    params=dict(
        log=True,
        risk=Decimal('4.0'),
        add_risk=Decimal('5.0'),
        percents=[Decimal(2.0), Decimal(4.0), Decimal(6.0), Decimal(8.0), Decimal(10.0), Decimal(12.0)],
        exit_percent={
            'def':Decimal('0.5')
        },
        tick_size={
            'SHIB1000USDT': common.fetch_tick_size(exchange, 'SHIB1000USDT'),
            'XRPUSDT': common.fetch_tick_size(exchange, 'XRPUSDT'),
            'DOGEUSDT': common.fetch_tick_size(exchange, 'DOGEUSDT'),
            'LPTUSDT': common.fetch_tick_size(exchange, 'LPTUSDT'),
            'INJUSDT': common.fetch_tick_size(exchange, 'INJUSDT'),
            'JASMYUSDT': common.fetch_tick_size(exchange, 'JASMYUSDT'),
            '1000PEPEUSDT': common.fetch_tick_size(exchange, '1000PEPEUSDT'),
        },
        step_size={
            'SHIB1000USDT': common.fetch_step_size(exchange, 'SHIB1000USDT'),
            'XRPUSDT': common.fetch_step_size(exchange, 'XRPUSDT'),
            'DOGEUSDT': common.fetch_step_size(exchange, 'DOGEUSDT'),
            'LPTUSDT': common.fetch_step_size(exchange, 'LPTUSDT'),
            'INJUSDT': common.fetch_step_size(exchange, 'INJUSDT'),
    ...
    def next(self):
        ...
        for i in range(0, len(self.pairs)):
            ...
            if current_position_size > 0:
                risk = self.p.add_risk
                exit_percent = self.p.exit_percent['def']
                exit_price = DataUtils.convert_to_decimal(self.getposition(self.pairs[i]).price) * (Decimal(1) + exit_percent / Decimal(100))
