...
class TrendFollowShortOpt(bt.Strategy):
    params = dict(
        high_band_length=5,
        low_band_length=20,
        high_band_const=50,
    ...
    def next(self):
        ...
        if mypos == 0:
            ...
            stop_price = adj_low_band + self.atr[0] * self.p.atr_const
            stop_price = int(stop_price / self.p.tick_size) * self.p.tick_size
            self.stop_price = stop_price

            qty = equity * self.p.risk / 100 / abs(adj_low_band-stop_price)
            if qty * adj_low_band / leverage >= equity:
                qty= equity * 0.98 / adj_low_band
            self.order=self.sell(exectype=bt.Order.Stop, data=self.datas[0], price=adj_low_band, size=qty)
            ...
            if self.closes[-1] < self.stop_price <= self.closes[0]:
                self.order=self.buy(exectype=bt.Order.Market, data=self.datas[0],size=mypos)
            else:
                self.order=self.buy(exectype=bt.Order.Stop, data=self.datas[0], size=mypos, price=adj_high_band)
...
if __name__ == '__main__':
    ...
    df = DataUtils.load_candle_data_as_df(data_path, DataUtils.COMPANY_BINANCE, 'ETHUSDT', pairs['ETHUSDT'])
    data = bt.feeds.PandasData(dataname=df, datetime='datetime')
    cerebro.adddata(data)  # 데이터 피드 추가

    # 최적화 찾기 추가
    # 너무 많은 변수 찾기는 메모리 부족을 일으킬 수 있음
    cerebro.optstrategy(TrendFollowShortOpt, log=False, high_band_length=range(5, 30, 5), low_band_length=range(10, 50, 5))
